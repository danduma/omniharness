...
export class GitWorkspaceManager extends StateManager<GitWorkspaceManagerState> {
  private api: GitWorkspaceApiClient | null;
  ...
  constructor(api: GitWorkspaceApiClient | null = null) {
    super(INITIAL_STATE);
    this.api = api;
  }

  configure(execute: RuntimeAPIs["git"]["execute"]) {
    this.api = (request) => execute(request) as Promise<GitWorkspaceApiResponse>;
  }

  private getApi() {
    if (!this.api) {
      throw new Error(t("git.workspace.error.generic"));
    }
    return this.api;
  }
  ...
  async loadStatus(projectPath: string) {
    ...
    try {
      const payload = await this.getApi()({ operation: "status", projectPath });
  ...
  private async runOperation(
    ...
    try {
      const payload = await this.getApi()(request);

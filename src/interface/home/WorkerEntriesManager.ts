...
export class WorkerEntriesManager {
  ...
  private load(input: Parameters<RuntimeAPIs["workers"]["listEntries"]>[0]) {
    if (!this.listEntries) {
      return Promise.reject(new Error("Worker entries are not connected to the runtime."));
    }
    return this.listEntries(input) as Promise<FetchResponse>;
  }
  ...
  private fetchForward(workerId: string, afterSeq: number): Promise<void> {
    ...
    const promise = this.load({ workerId, afterSeq }).then(
      (response) => {
  ...
  private applyFetchResult(
    ...
    for (const entry of incoming) {
      if (entry.seq === nextContiguous + 1) {
        ...
        // Gap. Don't include the entry; the next forward load (kicked off

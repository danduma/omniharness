...
export class ProjectMemoryPanelManager extends StateManager<ProjectMemoryPanelState> {
  ...
  private api: RuntimeAPIs["settings"]["projectMemory"] | null;

  constructor(api: RuntimeAPIs["settings"]["projectMemory"] | null = null) {
    super({ ...INITIAL_STATE });
    this.api = api;
  }

  configure(api: RuntimeAPIs["settings"]["projectMemory"]) {
    this.api = api;
  }

  private getApi() {
    if (!this.api) {
      throw new Error("Project memory is not connected to the runtime.");
    }
    return this.api;
  }
  ...
  async reloadList() {
    ...
    try {
      const data = await this.getApi().load({ projectPath }) as ListResponse;
      const current = this.getSnapshot();
  ...
  async toggleEnabled(next: boolean) {
    ...
    try {
      await this.getApi().save({ projectPath, enabled: next });

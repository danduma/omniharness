...
export const conversationHandoffs = sqliteTable('conversation_handoffs', {
  id: text('id').primaryKey(),
  sourceRunId: text('source_run_id').notNull().references(() => runs.id, { onDelete: 'cascade' }),
  sourceWorkerId: text('source_worker_id').references(() => workers.id, { onDelete: 'set null' }),
  targetRunId: text('target_run_id').references(() => runs.id, { onDelete: 'set null' }),
  forkedFromMessageId: text('forked_from_message_id'),
  reason: text('reason').notNull(),
  status: text('status').notNull(),
  revision: integer('revision').notNull().default(1),
  sourceSeq: integer('source_seq'),
  normalizedProjectPath: text('normalized_project_path').notNull(),
  workspaceFingerprint: text('workspace_fingerprint'),
  targetWorkerType: text('target_worker_type').notNull(),
  targetModel: text('target_model'),
  targetEffort: text('target_effort'),
  targetAccountId: text('target_account_id'),
  targetSelectionHash: text('target_selection_hash').notNull(),
  packetVersion: integer('packet_version'),
  artifactSeq: integer('artifact_seq'),
  packetHash: text('packet_hash'),
  packetPreview: text('packet_preview'),
  summarySource: text('summary_source'),
  operationId: text('operation_id'),
  launchClaimToken: text('launch_claim_token'),
  claimExpiresAt: integer('claim_expires_at', { mode: 'timestamp' }),
  retryOfHandoffId: text('retry_of_handoff_id'),
  lastError: text('last_error'),
  createdAt: integer('created_at', { mode: 'timestamp' }).notNull(),
  updatedAt: integer('updated_at', { mode: 'timestamp' }).notNull(),
  completedAt: integer('completed_at', { mode: 'timestamp' }),
}, (table) => [
  index('conversation_handoffs_source_idx').on(table.sourceRunId, table.updatedAt),
  index('conversation_handoffs_target_idx').on(table.targetRunId),
  index('conversation_handoffs_claim_idx').on(table.status, table.claimExpiresAt),
]);
...
export const accounts = sqliteTable('accounts', {
  ...
  lifecycleOperationKind: text('lifecycle_operation_kind'),
  lifecycleOperationOwner: text('lifecycle_operation_owner'),
  lifecycleOperationStartedAt: integer('lifecycle_operation_started_at', { mode: 'timestamp' }),
  lifecycleOperationDeadlineAt: integer('lifecycle_operation_deadline_at', { mode: 'timestamp' }),
  lifecycleOperationErrorCode: text('lifecycle_operation_error_code'),
  lifecyclePreviousStatus: text('lifecycle_previous_status'),
  lifecyclePreviousEnabled: integer('lifecycle_previous_enabled', { mode: 'boolean' }),
  createdAt: integer('created_at', { mode: 'timestamp' }).notNull(),
...
export const authSessions = sqliteTable('auth_sessions', {
  ...
  boundOrigin: text('bound_origin'),
  clientKind: text('client_kind').notNull().default('browser'),

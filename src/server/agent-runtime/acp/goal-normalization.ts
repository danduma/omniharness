...
export function isGoalRecord(value: unknown): value is Record<string, unknown> {
...
export function boundedGoalIdentityToken(value: string) {
  if (value.length <= 64 && /^[A-Za-z0-9._:-]+$/.test(value)) return value;
...
function normalizeProviderValidation(value: unknown): GoalValidationState | null | "invalid" {
  if (value === undefined || value === null) return null;
  if (!isGoalRecord(value)) return "invalid";
  const status = cleanString(value.status);
  if (!status || !["idle", "validating", "passed", "failed"].includes(status)) return "invalid";
  const rawMessage = value.message === undefined || value.message === null ? null : cleanString(value.message);
  if ((value.message !== undefined && value.message !== null && rawMessage === null) || (rawMessage?.length ?? 0) > GOAL_PLAN_ITEM_MAX_LENGTH) {
    return "invalid";
  }
  const updatedAt = cleanString(value.updatedAt);
  if (!updatedAt || !Number.isFinite(new Date(updatedAt).getTime())) return "invalid";
  return {
    status: status as GoalValidationState["status"],
    message: rawMessage ? redactGoalErrorMessage(rawMessage) : null,
    updatedAt,
  };
}

function capabilityInput(value: Record<string, unknown>) {
  const capabilities = isGoalRecord(value.capabilities) ? value.capabilities : {};
...
export function normalizeAcpGoalMetadata(value: unknown): AcpGoalMetadataResult {
  ...
  if (!isGoalRecord(value) || !isGoalRecord(value._meta) || !isGoalRecord(value._meta.goal)) {
  ...
  const validationState = normalizeProviderValidation(goal.validationState ?? goal.validation);
  if (validationState === "invalid") return { ok: false, reason: "invalid_validation" };
...
function normalizePlanItems(
  ...
  for (const [order, candidate] of entries.entries()) {
    if (!isGoalRecord(candidate)) return { ok: false, reason: "malformed", itemIndex: order };
    ...
    const metadata = isGoalRecord(candidate._meta) ? candidate._meta : {};
...
export function normalizeAcpGoalPlanUpdate(
  ...
  const envelopeFailure = inspectEnvelope(value);
  if (envelopeFailure) return { ok: false, reason: envelopeFailure };
  if (!isGoalRecord(value) || typeof value.sessionUpdate !== "string") return { ok: false, reason: "malformed" };
  ...
  if (value.sessionUpdate !== "plan_update" || !isGoalRecord(value.plan)) {
    return { ok: false, reason: "malformed" };
  }

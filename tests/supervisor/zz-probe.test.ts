...
describe("Supervisor worker spawn flow", () => {
  beforeEach(async () => {
    const pre = await db.all(sql`PRAGMA busy_timeout`);
    console.log("BT_BEFORE_CLEANUP=" + Number((pre[0] as any)?.timeout ?? -1));
    vi.clearAllMocks();
    ...
    const post = await db.all(sql`PRAGMA busy_timeout`);
    console.log("BT_AFTER_CLEANUP=" + Number((post[0] as any)?.timeout ?? -1));
  });
